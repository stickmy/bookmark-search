...
def add_doc(path, title, content):
    writer.add_document(title=title, path=path, content=content, id=path)
...
def main():
    for path, _ in pages.items():
        try:
            name = _['name']
            print('正在建立索引: {}'.format(name))
            if len(name.split('.')) > 1 and name.split('.')[-1] == 'pdf':
                add_doc(path, name, 'pdf内容木有加载~')
            else:
                html = requests.get(_['url'], headers=header, timeout=5)
                if html.status_code == requests.codes.ok:
                    add_doc(path, name, extract_pure_text(html))
                else:
                    add_doc(path, name, '网页内容未能正确加载~')
        except:
            print('请求错误')
